...
tellraw @s[scores={LANGUAGE=2}] {"text":"在被Golem炸出的一条小路的尽头中，你用火球炸倒了一座高塔，修复了断桥，随后你在Billgart塔楼中逛到昏天地暗，终于拿到了一颗时间宝石..","color":"dark_green"}
...
#tellraw @s[scores={LANGUAGE=2}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":":"},{"text":"[你在Billgart塔楼中逛的昏天地暗，终于拿到了一颗时间宝石-->]","color":"green","clickEvent":{"action":"run_command","value":"/function att2:dialogs/dialogs_review/mainquest_dialogs/gem1/rav2"},"hoverEvent":{"action":"show_text","value":"点击这里—回答1"}}]}
